//go:build windows && cgo && freemem && go1.20
// +build windows,cgo,freemem,go1.20
...
type mheap struct {
	...
	_              [136]struct {
		...
		_ uint64
		_ [cacheLineSize - (1+(2*((4*ptrSize)+8)))%cacheLineSize]byte
	}
	spanalloc             fixalloc
	cachealloc            fixalloc
...
func enumRuntimeMemory(h *mheap, m memoryMap) {
	m.add(uintptr(unsafe.Pointer(&h.pages.scav.chunks[0])))
	...
	m.add(h.specialReachableAlloc.chunk)
	m.add(h.specialReachableAlloc.inuse)
