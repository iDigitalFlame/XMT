...
var (
	// ErrEmptyString is an error returned from the Match function when
	// the calling Matcher is empty.
	ErrEmptyString = errors.New("matcher string cannot be empty")
...
func (s Matcher) Match() (Regexp, error) {
...
func (s Matcher) UnMatch() (Regexp, error) {
...
func (s Matcher) MatchEx(o bool) (Regexp, error) {
	if len(s) == 0 {
	...
	if m == nil || len(m) == 0 {
		if !o {
			return inverseRegexp(s), nil
		}
		return regexp.Compile(fmt.Sprintf(matchSingle, regexp.QuoteMeta(string(s))))
	}
	b := builders.Get().(*strings.Builder)
	defer resetBuffer(b)
	var l int
