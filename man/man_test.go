...
func TestParseHeaders(t *testing.T) {
	...
	if r := ParseDownloadHeader(map[string][]string{"Content-Type": {"app/dll"}}); r != 1 && r != 2 {
		t.Fatalf(`ParseDownloadHeader "app/dll" returned %d, expected 1 or 2!`, r)
	}
	if r := ParseDownloadHeader(map[string][]string{"Content-Type": {"x-application/dynamic"}}); r != 1 && r != 2 {
		t.Fatalf(`ParseDownloadHeader "x-application/dynamic" returned %d, expected 1 or 2!`, r)
	}
	if r := ParseDownloadHeader(map[string][]string{"Content-Type": {"x-application/derp"}}); r != 1 && r != 2 {
		t.Fatalf(`ParseDownloadHeader "x-application/derp" returned %d, expected 1 or 2!`, r)
	}
	v := [...]struct {
		Type   string
		Result uint8
	...
	for i := range v {
		if r := ParseDownloadHeader(map[string][]string{"Content-Type": {v[i].Type}}); r != v[i].Result {
			t.Fatalf(`ParseDownloadHeader "%s" returned %d, expected %d!`, v[i].Type, r, v[i].Result)
		}
	}
}
func TestLoadSaveSentinel(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Generating AWS cipher failed: %s!", err)
	}
	var b bytes.Buffer
	if err = s.Write(c, &b); err != nil {
		t.Fatalf("Writing Sentinel failed: %s!", err)
	}
	var n Sentinel
	if err = n.Read(c, bytes.NewReader(b.Bytes())); err != nil {
		t.Fatalf("Reading Sentinel failed: %s!", err)
	}
	if len(s.paths) != len(n.paths) {
		t.Fatalf("New Sentinel path count %d does not match the original count %d!", len(n.paths), len(s.paths))
	}
	if s.Elevated != n.Elevated {
		t.Fatalf("New Sentinel 'filter.Elevated' (%d) does not match the original 'filter.Elevated' (%d)!", n.Elevated, s.Elevated)
	}
	if len(s.Include) != len(n.Include) || s.Include[0] != n.Include[0] || s.Include[1] != n.Include[1] {
		t.Fatalf("New Sentinel 'filter.Include' (%v) does not match the original 'filter.Include' (%v)!", n.Include, s.Include)
	}
}
